...
export class CartComponent implements OnInit {
  ...
  loadBooks(){
    ...
    .subscribe((res:any)=>{
      ...
      for(let [i,val] of this.cartItemsList.entries()){
          this.totalPrice += this.cartItemsList[i]["Book"]["BPrice"]*this.cartItemsList[i]["Count"];
  ...
  increment(Bid:number,count:number){
    this.cartService.changeQuantity(Bid,count+1)
    .subscribe((res:any)=>{
      this.loadBooks();
      return res;
    });
  ...
  removeFromCart(BId:number){
    this.cartSubscription = this.cartService.removeFromCart(BId)
    .subscribe((res:any)=>{
      this.loadBooks();
      return res;
    });
